...
```bash
python -m torch.distributed.launch --nproc_per_node 8 --master_port 12345  main.py \
--cfg configs/swin_base_patch4_window7_224.yaml --data-path /imagenet --batch-size 4 --throughput --amp-opt-level O0 --zip \
--accumulation-steps 2 [--use-checkpoint]
```

```shell script
python -m torch.distributed.launch --nproc_per_node 1 --master_port 12347  main.py \
--cfg configs/swin_base_patch4_window7_224.yaml --data-path /imagenet --batch-size 4 \
 --zip --accumulation-steps 2 [--use-checkpoint]
```

```shell script
python3 -m oneflow.distributed.launch --nproc_per_node 8 --master_port 12345  main_ddp.py \
--cfg configs/swin_base_patch4_window7_224.yaml --data-path /imagenet --batch-size 64  --zip --accumulation-steps 2
```