...
def main(config):
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        train_one_epoch(config, model, optimizer, epoch, lr_scheduler, train_graph)
...
def train_one_epoch(config, model, optimizer, epoch, lr_scheduler, train_graph, mixup_fn = None):
    model.train()
    ...
    for idx in range(num_steps):
        ...
        loss = train_graph(image, targets).to_local()
