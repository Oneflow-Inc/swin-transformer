...
def main(config):
    ...
    if config.MODEL.RESUME:
        ...
        val_graph = EvalGraph(model=model)
        acc1, acc5, loss = validate(config, data_loader_val, model, val_graph)
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        train_one_epoch(config, model, data_loader_train, optimizer, epoch, lr_scheduler, train_graph, mixup_fn)
...
def train_one_epoch(config, model, data_loader, optimizer, epoch, lr_scheduler, train_graph, mixup_fn):
    model.train()
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        image = samples.to("cuda")
        label = targets.to("cuda")
        if mixup_fn is not None:
            image, label = mixup_fn(image, label)
        image = image.to_consistent(
...
@flow.no_grad()
def validate(config, data_loader, model, val_graph):
    criterion = flow.nn.CrossEntropyLoss()
    model.eval()
