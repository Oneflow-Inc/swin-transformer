...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        gh, gw = h // fh, w // fw
        num_patches = gh * gw
        self.embedding = nn.Conv2d(3, emb_dim, kernel_size=(fh, fw), stride=(fh, fw)).to_consistent(
            placement=flow.env.all_device_placement("cuda"), sbp=flow.sbp.split(0))
        # class token
        self.cls_token = nn.Parameter(flow.zeros(1, 1, emb_dim)).to_consistent(
            placement=flow.env.all_device_placement("cuda"), sbp=flow.sbp.split(0))
        ...
        ).to_consistent(placement=flow.env.all_device_placement("cuda"), sbp=flow.sbp.split(0))

        # classfier
        #        self.classifier = nn.Linear(emb_dim, num_classes).to_consistent(placement=flow.env.all_device_placement("cuda"), sbp=flow.sbp.broadcast)
        #        self.classifier = self.classifier.to_consistent(placement=flow.env.all_device_placement("cuda"), sbp=flow.sbp.split(0))
        self.classifier = nn.Linear(emb_dim, num_classes).to_consistent(
            placement=flow.placement(device_type="cuda",
                                     device_ids={0: [0, 1, 2, 3, 4, 5, 6, 7], 1: [0, 1, 2, 3, 4, 5, 6, 7]},
                                     hierarchy=(2, 8)),
    ...
    def forward(self, x):
        ...
        # transformer
        feat = self.transformer(emb).to_consistent(
            placement=flow.placement(device_type="cuda",
                                     device_ids={0: [0, 1, 2, 3, 4, 5, 6, 7], 1: [0, 1, 2, 3, 4, 5, 6, 7]},
                                     hierarchy=(2, 8)),
            sbp=[flow.sbp.broadcast, flow.sbp.split(0)])
