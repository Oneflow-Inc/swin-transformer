...
from flowvision.layers import to_2tuple
...
def drop_path(x, drop_prob: float = 0.5, training: bool = False):
    ...
    random_tensor = keep_prob + torch.rand(*shape, dtype=x.dtype,
                                           placement=torch.placement("cuda", {0: range(torch.env.get_world_size())}),
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = torch.gather_nd(self.relative_position_bias_table,self.relative_position_index.view(-1).reshape(-1,1)
        ).view(
    ...
    def forward(self, x):
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                # x = torch.checkpoint.checkpoint(blk, x)
            else:
                x = blk(x)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        dpr = [x.item() for x in torch.Tensor(np.linspace(0, drop_path_rate, sum(depths)))]  # stochastic depth decay rule
