...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler):
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        outputs = model(samples.to_consistent(placement=flow.env.all_device_placement("cuda"), sbp=flow.sbp.split(0)))
        ...
        if config.TRAIN.ACCUMULATION_STEPS > 1:
            ...
            loss.backward()
            if config.TRAIN.CLIP_GRAD:
                grad_norm = flow.nn.utils.clip_grad_norm_(model.parameters().to_local(), config.TRAIN.CLIP_GRAD)
            else:
